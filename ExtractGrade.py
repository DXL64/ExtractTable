...
def pipeline(path):
    ...
    for i in range(number_of_pages):
        img[i] = pdf[i].render_to(
            ...
            # no crop (form: left, right, bottom, top)
            crop=(0, 0, 0, 0),
            greyscale=False,                   # coloured output
            # fill bitmap with white background before rendering (form: RGBA)
    ...
    for i in range(number_of_pages):
        ...
        thresh, img_bin[i] = cv2.threshold(
            img[i], 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        # inverting the image
    ...
    for i in range(number_of_pages):
        img_vh[i] = cv2.addWeighted(
            vertical_lines[i], 0.5, horizontal_lines[i], 0.5, 0.0)
        # Eroding and thesholding the image
        img_vh[i] = cv2.erode(~img_vh[i], kernel, iterations=2)
        thresh, img_vh[i] = cv2.threshold(
            img_vh[i], 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    ...
    for i in range(number_of_pages):
        contours[i], hierarchy = cv2.findContours(
            img_vh[i], cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for k in range(number_of_pages):
        ...
        contours[k], boundingBoxes[k] = sort_contours(
            contours[k], method='top-to-bottom')

    heights = [None]*number_of_pages
    mean = [None]*number_of_pages
    for k in range(number_of_pages):
        # Creating a list of heights for all detected boxes
        heights[k] = [boundingBoxes[k][i][3]
                      for i in range(len(boundingBoxes[k]))]
        # Get mean of heights
    ...
    for k in range(number_of_pages):
        ...
        if len(row[k]) > 0:
            center[k] = [int(row[k][i][j][0]+row[k][i][j][2]/2)
    ...
    for z in range(number_of_pages):
        ...
        for i in range(len(finalboxes[z])):
            ...
            for j in range(len(finalboxes[z][i])):
                if cnt == 0 or cnt == 2 or cnt == 3 or cnt == 4:
                    cnt = cnt+1
                    continue
                cnt = cnt+1
                inner = ''
                if (len(finalboxes[z][i][j]) == 0):
                    ...
                    for k in range(len(finalboxes[z][i][j])):
                        y, x, w, h = finalboxes[z][i][j][k][0], finalboxes[z][i][j][
                            k][1], finalboxes[z][i][j][k][2], finalboxes[z][i][j][k][3]
    ...
    arr = np.array(outer)
    dataframe = pd.DataFrame(arr.reshape(-1, countcol[0]-4))
    for i in range(countcol[0]-4):
        if (i != 0):
            dataframe[i] = pd.to_numeric(
                dataframe[i], errors='coerce')
    if (countcol[0] == 7):
        dataframe = dataframe.dropna(subset=[1, 2])
    if (countcol[0] == 8):
        dataframe = dataframe.dropna(subset=[1, 2, 3])

    dataframe = dataframe.reset_index(drop=True)
    dataframe['linkPDF'] = pdf_link
    dataframe.rename(
        columns={0: 'studentCode', 1: 'midterm', 2: 'final', 3: 'total'}, inplace=True)
    for i in range(len(dataframe)):
        if (dataframe['midterm'][i] > 10):
            dataframe['midterm'][i] = dataframe['midterm'][i] - 10
        if (dataframe['final'][i] > 10):
            dataframe['final'][i] = dataframe['final'][i] - 10
        if (dataframe['total'][i] > 10):
            dataframe['total'][i] = dataframe['total'][i] - 10
    print(dataframe)
...
if __name__ == '__main__':
    ...
    pdf = 'https://drive.google.com/file/d/13joEKMaG8ttxGePnwOBTW60I5bcizEID/'
    pipeline(opt.path)
